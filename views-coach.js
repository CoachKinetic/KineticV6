...
export function coachAtt(){
  ...
  const p=Object.values(APP.attState).filter(v=>v==='present').length;
  return `
  <div class="alert info">📋 Taking attendance for <strong>${cls.name}</strong></div>
  <div class="sec-hdr"><h3>${aths.length} Athletes</h3>
    <div style="display:flex;gap:8px;">
      <button class="btn" onclick="window.K.markAll('present')">All Present</button>
      <button class="btn" onclick="window.K.markAll('absent')">All Absent</button>
      <button class="btn primary" onclick="window.K.saveAtt()" ${APP.attSaved?'disabled':''}>${APP.attSaved?'✓ Saved':'Save ☁️'}</button>
    </div>
  </div>
  <div class="card">
    <div class="card-hdr"><h4>${cls.name}</h4><div style="display:flex;gap:6px;"><span class="pill present" id="attP">${p} Present</span><span class="pill absent" id="attA">${aths.length-p} Absent</span></div></div>
    <div class="card-body">
      ${aths.length===0?`<div style="padding:24px;text-align:center;color:var(--t3);">No athletes in this class yet.</div>`
      :aths.map((a,i)=>`<div class="att-row" id="ar${i}">
        <div class="mini-av">${ini(a.name)}</div>
        <div class="att-name">${a.name}</div>
        <div class="att-meta">${a.level||'Level 1'}</div>
        <div class="att-btns">
          <button class="att-btn ${APP.attState[i]==='present'?'sel-p':''}" onclick="window.K.setAtt(${i},'present')">Present</button>
          <button class="att-btn ${APP.attState[i]==='absent'?'sel-a':''}" onclick="window.K.setAtt(${i},'absent')">Absent</button>
        </div>
      </div>`).join('')}
    </div>
  </div>
  ${APP.attSaved?`<button class="btn primary full" onclick="window.K.nav('coachSkills')">Attendance Done → Skills</button>`:''}`;
}
        ...
        ${evts.map(evt=>{
          ...
          return `<div class="evt-hdr">${evt}</div>${es.map(sk=>{
            ...
            return `<div class="skill-row"><span class="skill-nm" onclick="window.K.nav('skill_${sk.id}')">${sk.name}</span>
              <div class="ssbs">
                <button class="ssb ${st==='nr'?'sel-nr':''}" onclick="window.K.setSkill(${ai},'${sk.id}','nr')">NR</button>
                <button class="ssb ${st==='ip'?'sel-ip':''}" onclick="window.K.setSkill(${ai},'${sk.id}','ip')">IP</button>
                <button class="ssb ${st==='m'?'sel-m':''}" onclick="window.K.setSkill(${ai},'${sk.id}','m')">✓</button>
              </div></div>`;
          }).join('')}`;
        }).join('')}
      </div>
